...
export const FACTORY_ADDRESS = '0x459bA2f7871336e95B78990C0e20636A54fdf6D2'.toLowerCase() // TODO: update address
...
// rebass tokens, don't count in tracked volume
...
export function createLiquidityPosition(exchange: Address, user: Address): LiquidityPosition {
  ...
  if (liquidityTokenBalance === null) {
    ...
    if (pair) {
      log.warning(`[===prince===] Pair.load ${exchange.toHexString()}`, [``])
    } else {
...
export function createLiquiditySnapshot(position: LiquidityPosition, event: ethereum.Event): void {
  ...
  if (pair) {
    log.warning(`[===prince===] Pair.load ${position.pair}`, [``])
  } else {
    log.warning(`[===prince===] Pair.load pair is null`, [``])
  }
  if (pair && bundle) {
